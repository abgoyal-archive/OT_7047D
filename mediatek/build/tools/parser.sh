 #!/bin/bash
#-------------------------------------------------------------------------------
# Purpose:     parser res check result and output table format, such csv or excel
# Author:      Jibin Zhang
# Created:     2013-06-20
# update:      2013-06-28
#-------------------------------------------------------------------------------
...
function gen_fin_result()
{
    ...
    if [ ]; then
        ...
        awk -F' ' '{print $0}' $MID_FILE_TYPE > $FIN_FILE_NAME
        G_NUM=`wc -w $MID_FILE_TYPE|awk -F' ' '{print $1}'`
    ...
    awk -F'[: ]' '$0 ~ /\[UnusedResources]/ {if ($2!="") {print $1,$2,$8,$13} else {print $1,"(File)",$7,$12}}' $4"/$3" >> $FIN_FILE_NAME
}
