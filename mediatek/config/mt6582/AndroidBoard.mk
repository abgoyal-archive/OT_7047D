...
_yaris_xl_compass_cfg := $(MTK_ROOT_CONFIG_OUT)/configs/accel_nvm
ifeq ("yaris_xl","$(TARGET_PRODUCT)")
ifneq ($(wildcard $(_yaris_xl_compass_cfg)),)
PRODUCT_COPY_FILES += $(_yaris_xl_compass_cfg):system/etc/accel_nvm
endif
endif


_init_usb_rc_conf := $(LOCAL_PATH)/init.usb.rc
_init_panasonic_usb_rc_conf := $(LOCAL_PATH)/init.panasonic.usb.rc
ifeq ($(PANASONIC_BUILD),true)
PRODUCT_COPY_FILES += $(LOCAL_PATH)/init.panasonic.usb.rc:root/init.usb.rc
else
PRODUCT_COPY_FILES += $(LOCAL_PATH)/init.usb.rc:root/init.usb.rc
endif
