...
#ifdef	ECLIPSE_KEY_BACKLIGHT_SET	
				upmu_set_rg_drv_32k_ck_pdn(0x0); // Disable power down  
				upmu_set_rg_isink3_ck_pdn(0);
				upmu_set_rg_isink3_ck_sel(0);
				upmu_set_isink_ch3_mode(PMIC_PWM_0);
	...
	*/	
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_PWM:
			...
			{
				if(level == 0)
				{	printk("guoyanchun           mt_mt65xx_led_set_cust         NLED_OFF\n");
					if(!strcmp(cust->name,"red"))
						{
							mt_set_gpio_out(GPIO41,GPIO_OUT_ONE);//for power saving
						}
				...
				}else
				{
					if(!strcmp(cust->name,"red"))
						{
							mt_set_gpio_out(GPIO41,GPIO_OUT_ZERO);//for power saving
						}
					led_tmp_setting.nled_mode = NLED_ON;
				}
				if(!strcmp(cust->name,"red"))
				{
					mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,level, 0);
				}
				else
