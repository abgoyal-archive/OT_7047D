...
#if defined(YARISXXL_PRETECT_BACKLIGHT_IC) | defined(ECLIPSE_PRETECT_BACKLIGHT_IC)
extern char *saved_command_line;
static unsigned int backlightOn_flag = 1;  // default value should let backlight turn on
BOOL yarisxxl_pretect_backlight_ic_func(void)
{
	BOOL ret = FALSE;
	char *p;

	p = strstr(saved_command_line, "backlightOn=");
	if(p == NULL){
		return TRUE; //no flag means no need this function.
	}
...
#if defined(YARISXXL_PRETECT_BACKLIGHT_IC) | defined(ECLIPSE_PRETECT_BACKLIGHT_IC)
	//printk("%s backlightOn_flag = %d\n", __func__, backlightOn_flag);
	if(backlightOn_flag == 0)
		return;
#endif
...
#if defined(YARISXXL_PRETECT_BACKLIGHT_IC) | defined(ECLIPSE_PRETECT_BACKLIGHT_IC)
	backlightOn_flag = yarisxxl_pretect_backlight_ic_func(); // for pretect yarisxxl Lcd backlight ic. 
#endif
