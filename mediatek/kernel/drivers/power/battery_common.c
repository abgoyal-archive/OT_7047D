...
#include <mach/mt_boot.h>
#include "../../../../../mediatek/platform/mt6582/kernel/drivers/power/fan5405.h"
...
char proc_bat_data[32];  
...
#if defined(YARISXL_CHARGING_TEMPERATURE_PLOICY) || defined(YARISXLTD_CHARGING_TEMPERATURE_PLOICY)
...
#endif
#if defined(YARISXXL_CHARGING_TEMPERATURE_PLOICY)
 int charge_temperature_above_step1 = 0;
 int charge_temperature_above_step2 = 0;
 int charge_temperature_above_step1_valid = 0;
 int charge_temperature_above_step2_valid = 0;
 extern  Full_status_check;
#endif
#if defined(ECLIPSE_CHARGING_TEMPERATURE_PLOICY)
 int charge_temperature_above_step1 = 0;
 int charge_temperature_above_step2 = 0;
...
#if defined(YARIS_XL_THREAD_INIT_FLAG)
...
#define CHG_MAX_LISHEN_BATT_ID 1000
#define CHG_MIN_LISHEN_BATT_ID 800
#else
#define CHG_MIN_BYD_BATT_ID  400
#define CHG_MAX_BYD_BATT_ID  600
#define CHG_MIN_SCUD_BATT_ID  1300
...
/* start, add by ozy for cdp reboot */
static struct hrtimer charger_check_timer;
static kal_bool charger_check_flag = KAL_FALSE;
/* end, add by ozy for cdp reboot */
#ifdef NEW_POLICY_TEST
INT32 battery_temperature_test = 25;
int battery_temperature_test_enable = 0;
#endif
...
void check_battery_id(void){
  ...
  //printk("%s: bat_id_vol:%d!\n", __func__, bat_id_vol);
	...
	else if ((bat_id_vol < CHG_MAX_LISHEN_BATT_ID)&&(bat_id_vol > CHG_MIN_LISHEN_BATT_ID))
	{
		BMT_status.bat_id_valid = 1;
		byd_sucd_batt_id=3; //LISHEN
		printk("battery id LISHEN %d,between %d and %d !\n", bat_id_vol,CHG_MIN_LISHEN_BATT_ID,CHG_MAX_LISHEN_BATT_ID);
...
static ssize_t show_Temperature_Test(struct device *dev,struct device_attribute *attr, char *buf)
{
    battery_xlog_printk(BAT_LOG_CRTI, "[EM] show_ADC_Charger_Voltage : %d\n", BMT_status.charger_vol);
    return sprintf(buf, "%d\n", BMT_status.charger_vol);
}
static ssize_t store_Temperature_Test(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    //battery_xlog_printk(BAT_LOG_CRTI, "[EM] Not Support Write Function\n");
	char *pvalue = NULL;
	battery_temperature_test = simple_strtoul(buf,&pvalue,0);
	printk("CHENFEI battery_temperature_test = %d \n",battery_temperature_test);
	if(battery_temperature_test >= -20 && battery_temperature_test <=60)
	{
		battery_temperature_test_enable = 1;
	}
    return size;
}
static DEVICE_ATTR(Tempreature_Test, 0664, show_Temperature_Test, store_Temperature_Test);
#endif
...
//add by wangping for yarisxxl japan None Standard charger start
#ifdef YARISXXL
static ssize_t show_jp_charger_flag(struct device *dev,struct device_attribute *attr, char *buf)
{
    battery_xlog_printk(BAT_LOG_CRTI, "[EM] jp_flag : %d\n", g_jp_flag);
    return sprintf(buf, "%u\n", g_jp_flag);
}
static ssize_t store_jp_charger_flag(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{	
	char *pvalue = NULL;
	unsigned int flag = 0;
	if(buf != NULL && size !=0)
	{
		flag = simple_strtoul(buf,&pvalue,10);
		
	}
	
	g_jp_flag = flag;
/*
    	if((g_jp_flag == 1) && (BMT_status.charger_type == NONSTANDARD_CHARGER)) //jp charger invoke select_charging_curret() function;
    	{
		...
		battery_charging_control(CHARGING_CMD_SET_CURRENT,&g_temp_CC_value);
    		battery_xlog_printk(BAT_LOG_CRTI, "[EM] xxxxxxjp charger need invoke the select_charging_curret function"); 
    	}
*/    	
    	battery_xlog_printk(BAT_LOG_CRTI, "[EM] set the jp charger flag:%d\n",g_jp_flag); 	
    return size;
}
static DEVICE_ATTR(jp_charger_flag, 0664, show_jp_charger_flag, store_jp_charger_flag);
#endif
//add by wangping for japan None Standard charger end

#if defined(YARISXL) || defined(YARISXLTD)
#ifdef BATT_ID_CHECK_SUPPORT
static ssize_t show_battery_id(struct device *dev,struct device_attribute *attr, char *buf)
{
	kal_int32 battery_id=g_battery_id;
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] battery id : %d\n", battery_id);
	return sprintf(buf, "%u\n", battery_id);
}
static ssize_t store_battery_id(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	kal_int32 battery_id=0;
	int rc=0;
	rc=kstrtouint(buf,10,&battery_id);
	g_battery_id=battery_id;
	return size;
}
static DEVICE_ATTR(BATT_ID, 0664, show_battery_id, store_battery_id);
#endif
#endif
...
{ 
	...
	#ifdef NEW_POLICY_TEST
	if(battery_temperature_test_enable)
	BMT_status.temperature = battery_temperature_test;
	#endif
...
{	
    ...
    #if defined(YARISXL_CHARGING_TEMPERATURE_PLOICY) || (YARISXLTD_CHARGING_TEMPERATURE_PLOICY)
    ...
    #elif defined(YARISXXL_CHARGING_TEMPERATURE_PLOICY)
    if(BMT_status.temperature >= CHARGE_TEMPERATURE_STEP1)
    {
        charge_temperature_above_step1 = 1;
    }
	...
	#elif defined(ECLIPSE_CHARGING_TEMPERATURE_PLOICY)
    if(BMT_status.temperature >= CHARGE_TEMPERATURE_STEP1)
    {
        charge_temperature_above_step1 = 1;
    }
...
#endif
#if defined(YARISXLTD_CHARGING_TEMPERATURE_PLOICY) || (YARISXL_CHARGING_TEMPERATURE_PLOICY)
	if(charge_temperature_above_step2 == 1)
	{
	...
	#ifdef YARISXXL
	else if((BMT_status.temperature < MIN_CHARGE_TEMPERATURE+2)&&(low_protect !=0) )
	{
		low_protect = 1;
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Under Temperature or NTC fail !!\n\r");
		status = PMU_STATUS_FAIL;
...
#define JRD_FAST_FULL_CHARGING
#ifdef JRD_FAST_FULL_CHARGING
/*
 * when reach soc 99, to happy users, force
 * soc 100 after *timeout*
 */
#define FULL_CHARGING_TIMEOUT (15*60)
#define TOPOFF_CHG_VOLTAGE (4270)
static int g_top_off_time = 0;
static void fast_full_charging()
{
	if(KAL_TRUE == upmu_is_chr_det())
	{
		if(BMT_status.SOC == 99 && BMT_status.bat_vol >= TOPOFF_CHG_VOLTAGE)
		{
			g_top_off_time += BAT_TASK_PERIOD;
			printk("Jrd fast_full_charging time = %d\n", g_top_off_time);
			if(g_top_off_time > FULL_CHARGING_TIMEOUT)
			{
				BMT_status.bat_full = KAL_FALSE;
				BMT_status.bat_in_recharging_state = KAL_TRUE;
				printk("Jrd fast_full_charging enabled\n");
			}
		}
			else if(g_top_off_time != 0)
			{
				g_top_off_time = 0;
				printk("Jrd fast_full_charging time reset\n");
			}
	}
	else if(g_top_off_time != 0)
	{
...
static void weak_charger_detect(void)
{
	...
	kal_uint8 v5=0;
	int j = 0;
	//battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,0x20);
	#if defined(YARIS_XXL_WEAK_CHARGER)
	fan5405_config_interface_liao(0x04,0x20);
	#else
	fan5405_config_interface_liao(0x04,0x0);
	#endif
	...
	fan5405_config_interface_liao(0x04,0x7a);
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	#endif
	#endif
	#ifdef BATTERY_PERCENT_90
	if((BMT_status.bat_vol >= 4190) && (BMT_status.SOC >= 90))
	{
		BMT_status.bat_charging_state = CHR_ERROR;
		return;
	}
...
{
    #if defined(YARISXXL) || defined(ECLIPSE)
	int count = 0;
    #endif
	...
	//redetect chg. type while insert usb slowly
	while((count < 6) && (CHR_Type_num == NONSTANDARD_CHARGER || CHR_Type_num>= APPLE_2_1A_CHARGER))
	{
	    count++;
	    battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE,&CHR_Type_num);
...
#ifdef YARISXXL
extern void tpd_usb_plugin(int plugin,int force);
#endif
static void mt_battery_charger_detect_check(void)
{
	#if defined(YARISXXL)|| defined(ECLIPSE)
	kal_uint32 temp_charging_enable;
	#endif
    if( upmu_is_chr_det() == KAL_TRUE )
    {
	#ifdef YARISXXL
	...
	{
		tpd_usb_plugin(1,0); //decrease the touch panel sense for charging
	}
	#endif
    	#if defined(YARISXXL)|| defined(ECLIPSE)
    	hz_mode_need = 1;
	...
	if(tpd_register_flag == 1)
	{
	    tpd_usb_plugin(0,0);
					...
					if(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
					{
						...
						{
						temp_charging_enable = KAL_FALSE;
						battery_charging_control(CHARGING_CMD_ENABLE,&temp_charging_enable);
						}
...
{
	printk("[do_chrdet_int_task] g_bat_init_flag == %d,g_thread_init_flag ==%d\n",g_bat_init_flag,g_thread_init_flag);
	#if defined(YARISXXL) || defined(ECLIPSE)
	kal_uint32 temp_charging_enable;
	#endif
#if defined(YARIS_XL_THREAD_INIT_FLAG)
    if((g_bat_init_flag == KAL_TRUE) && (g_thread_init_flag == KAL_TRUE))
#elif defined(YARISXXL)
    if((g_bat_init_flag == KAL_TRUE) && (g_thread_init_flag == KAL_TRUE))
...
/* start, add by ozy for cdp reboot */
enum hrtimer_restart charger_check_hrtimer_func(struct hrtimer *timer)
{
    g_thread_init_flag = KAL_TRUE;
    
    return HRTIMER_NORESTART;
}
void charger_check_hrtimer_init(void)
{
    ktime_t ktime;

    ktime = ktime_set(5, 0);	
    hrtimer_init(&charger_check_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    charger_check_timer.function = charger_check_hrtimer_func;    
    hrtimer_start(&charger_check_timer, ktime, HRTIMER_MODE_REL);

    battery_xlog_printk(BAT_LOG_CRTI, "charger_check_hrtimer_init : done\n" );
}
/* end, add by ozy for cdp reboot */
...
void BAT_thread(void)
{
    ...
    if(battery_meter_initilized == KAL_FALSE)
    {
		...
		BMT_status.ICharging += battery_meter_get_charging_current();
		BMT_status.ICharging += battery_meter_get_charging_current();
		BMT_status.ICharging = BMT_status.ICharging/3;
    	#endif
		#ifdef ECLIPSE
    	BMT_status.ICharging = battery_meter_get_charging_current();
		BMT_status.ICharging += battery_meter_get_charging_current();
...
int bat_thread_kthread(void *x)
{
    ...
    while (1) {               
        ...
        if(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
        {
	    if(charger_check_flag == KAL_FALSE)
	    {
		charger_check_hrtimer_init();
	       charger_check_flag = KAL_TRUE;
	    }
        }
	 else
	 {
...
static int battery_probe(struct platform_device *dev)    
{
    ...
    /* For EM */
	{
		...
		#ifdef NEW_POLICY_TEST
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Tempreature_Test);
		#endif
...
#ifdef YARISXXL	    
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_jp_charger_flag);
#endif
	    #if defined(YARISXL) || defined(YARISXLTD)
	    #ifdef BATT_ID_CHECK_SUPPORT
