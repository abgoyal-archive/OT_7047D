...
int BattThermistorConverTemp(int Res)
{
	...
	BATT_TEMPERATURE *Batt_Temperature_Table_ID = NULL;
	if(byd_sucd_batt_id == 1)
		{
			Batt_Temperature_Table_ID = Batt_Temperature_Table_1;
		}
	else if(byd_sucd_batt_id == 2)
		{
			Batt_Temperature_Table_ID = Batt_Temperature_Table_2;
		}
	else
		{
			Batt_Temperature_Table_ID = Batt_Temperature_Table_3;
		}
	#if DEBUG_NTC
		for(i = 0;i<=16;i++)
			{
				printk("POWER/BATTERY:NTC: Temp %d and NTC Value %d \n",Batt_Temperature_Table_ID[i].BatteryTemp,Batt_Temperature_Table_ID[i].TemperatureR);
			}
	#endif 
	if(Res>=Batt_Temperature_Table_ID[0].TemperatureR)
		{
			TBatt_Value = -20;
		}
		else if(Res<=Batt_Temperature_Table_ID[16].TemperatureR)
		{
			TBatt_Value = 60;
		}
		else
		{
			RES1=Batt_Temperature_Table_ID[0].TemperatureR;
			TMP1=Batt_Temperature_Table_ID[0].BatteryTemp;
	
			for(i=0;i<=16;i++)
			{
				if(Res>=Batt_Temperature_Table_ID[i].TemperatureR)
				{
					RES2=Batt_Temperature_Table_ID[i].TemperatureR;
					TMP2=Batt_Temperature_Table_ID[i].BatteryTemp;
					break;
				}
				else
				{
					RES1=Batt_Temperature_Table_ID[i].TemperatureR;
					TMP1=Batt_Temperature_Table_ID[i].BatteryTemp;
				}
			}
...
extern char* saved_command_line;  //add by jiangtao for pr667465
...
/******************add by jiangtao for pr667465**************/
       bm_print(BM_LOG_CRTI, "[oam_init_inf211212] vol_bat=%d, gFG_voltage=%d, \n",vol_bat,gFG_voltage);
	{
		char* p=NULL;
		unsigned int vbat_lk=0;
		unsigned int saved_percent = 0;
		//unsigned int vbat_lk_capacity_by_v = 0;
	
		p = strstr(saved_command_line, "vbat=");
		if(p)
		{
			bm_print(BM_LOG_CRTI, "will start to get vbat_lk \n");
			p=p+5;
			vbat_lk = simple_strtol(p, NULL, 10);
		}
		//saved_percent = get_rtc_spare_fg_value();
		//vbat_lk_capacity_by_v = fgauge_read_capacity_by_v(vbat_lk);

		if((vbat_lk != 0))//&&(saved_percent!=0))
		{
			gFG_voltage = vbat_lk;
			bm_print(BM_LOG_CRTI, "[oam_init_inf1212121] vol_bat=%d, gFG_voltage=%d, vbat_lk=%d,\n",vol_bat,gFG_voltage,vbat_lk);
		}
		else
		{
			bm_print(BM_LOG_CRTI, "can not get vbat_lk \n");
		}
	}
/************end*************************/
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
...
#if defined (JRD_FUEL_GAUGE_OPTIMIZE)
    oam_r_1 +=15;
    oam_r_2 +=15;
 #endif
 		...
 		if(BMT_status.bat_vol > 3550) 
 			{
 				if (oam_i_2 < 2000) 
					...
					vol_bat -= 50;
				else if(oam_i_2 < 9000)
					vol_bat -= 55;
				else
					vol_bat -= 70;
				printk("TCThychu  ******* vol_bat > 3550 \n");
 			}
		else if (BMT_status.bat_vol > 3480)
			{
				if (oam_i_2 < 2000) 
					vol_bat -= 0;
				else if(oam_i_2 < 4000)
