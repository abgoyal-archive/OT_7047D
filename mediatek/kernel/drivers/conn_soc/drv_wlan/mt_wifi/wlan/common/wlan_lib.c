    ...
    )
{
    ...
    do {
        if(kalIsCardRemoved(prAdapter->prGlueInfo) == TRUE
                || fgIsBusAccessFailed == TRUE) {
            u4Status = WLAN_STATUS_FAILURE;
	    DBGLOG(INIT,TRACE,("######wlanImageSectionDownloadStatus(): card is removed\n"));	
        }
        else if(nicRxWaitResponse(prAdapter,
                    0,
                    aucBuffer,
            ...
            u4Status = WLAN_STATUS_FAILURE;
	    DBGLOG(INIT,TRACE,("######wlanImageSectionDownloadStatus(): timeout\n"));	
        }
        else {
            prInitHifRxHeader = (P_INIT_HIF_RX_HEADER_T) aucBuffer;
            ...
            if(prInitHifRxHeader->rInitWifiEvent.ucEID != INIT_EVENT_ID_CMD_RESULT) {
                u4Status = WLAN_STATUS_FAILURE;
		DBGLOG(INIT,TRACE,("######wlanImageSectionDownloadStatus(): cmd error\n"));	
            }
            else if(prInitHifRxHeader->rInitWifiEvent.ucSeqNum != ucCmdSeqNum) {
                u4Status = WLAN_STATUS_FAILURE;
		DBGLOG(INIT,TRACE,("######wlanImageSectionDownloadStatus(): cmd sequence error\n"));
            }
            else {
                ...
                if(prEventCmdResult->ucStatus != 0) { // 0 for download success
                    /*
                            ...
                            4: unknown CMD
                        */
                    DBGLOG(INIT, TRACE, ("######wlanImageSectionDownloadStatus(): Read Response status error = %d\n",
                            prEventCmdResult->ucStatus));
                    u4Status = WLAN_STATUS_FAILURE;
		    DBGLOG(INIT,TRACE,("######wlanImageSectionDownloadStatus(): cmd status error\n"));	
                }
                else {
                    u4Status = WLAN_STATUS_SUCCESS;
                }
