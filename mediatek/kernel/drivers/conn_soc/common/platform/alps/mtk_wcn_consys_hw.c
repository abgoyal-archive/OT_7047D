...
INT32 mtk_wcn_consys_hw_reg_ctrl(UINT32 on,UINT32 co_clock_en)
{
	...
	if(on)
	{
		...
		WMT_DBG_FUNC("reg uump:CONSYS_CPU_SW_RST_REG(0x%x)\n",CONSYS_REG_READ(CONSYS_CPU_SW_RST_REG));
#endif
		...
		while (retry-- > 0)
		{
			WMT_DBG_FUNC("CONSYS_CHIP_ID_REG(0x%08x)",CONSYS_REG_READ(CONSYS_CHIP_ID_REG));
...
INT32 mtk_wcn_consys_hw_restore(struct device *device)
{
	...
	UINT32 addrfromWcp2 = 0;
	UINT32 addrPhy = 0;

	/*set MPU for EMI share Memory*/
	WMT_INFO_FUNC("setting MPU for EMI share memory\n");
	emi_mpu_set_region_protection(get_CONNSYS_start() + get_CONNSYS_size()/2,
		get_CONNSYS_start() + get_CONNSYS_size(),
		5,
		SET_ACCESS_PERMISSON(FORBIDDEN,NO_PROTECTION,FORBIDDEN,NO_PROTECTION));
	
	addrfromWcp2 = get_CONNSYS_start();
	WMT_INFO_FUNC("get consys start phy address(0x%x)\n",addrfromWcp2);

	/*consys to ap emi remapping register:10001310, cal remapping address*/
	addrPhy = (addrfromWcp2 & 0xFFF00000) >> 20;
	...
	WMT_INFO_FUNC("CONSYS_EMI_MAPPING dump(0x%08x)\n",CONSYS_REG_READ(CONSYS_EMI_MAPPING));
#if 1
	pEmibaseaddr = ioremap_nocache(addrfromWcp2 + CONSYS_EMI_AP_PHY_OFFSET,CONSYS_EMI_MEM_SIZE);
#else
...
INT32 mtk_wcn_consys_hw_init()
{
	...
	WMT_INFO_FUNC("register connsys restore cb for complying with IPOH function\n");
