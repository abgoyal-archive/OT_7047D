...
#define YARISXXL_TV_VERSION_CHECK
#ifdef YARISXXL_TV_VERSION_CHECK
#include "../../../../../kernel/drivers/auxadc/mt_auxadc.h"
int tv_version = 0; // default is not tv version.
#endif

//====================================
#define TP_DEBUG
#if defined(TP_DEBUG)
#define TPD_DEBUG(fmt, arg...)  printk("[tpd debug:5406]" "[%s]" fmt "\r\n", __FUNCTION__ ,##arg)
#define TPD_DMESG(fmt, arg...)  printk("[tpd dmesg:5406]" "[%s]" fmt "\r\n", __FUNCTION__ ,##arg)
#else
...
//BIEL module compatible design.
#define TS_TYPE_BIEL	0x3b
...
struct touch_info {
 ...
 static const struct i2c_device_id ft5406_tpd_id[] = {{"ft5406",0},{}};
 //unsigned short force[] = {0,0x70,I2C_CLIENT_END,I2C_CLIENT_END}; 
 //static const unsigned short * const forces[] = { force, NULL };
 //static struct i2c_client_address_data addr_data = { .forces = forces, };
 static struct i2c_board_info __initdata ft5406_i2c_tpd={ I2C_BOARD_INFO("ft5406", (0x70>>1))};
 
 
 static struct i2c_driver tpd_i2c_driver = {
  .driver = {
	 .name = "ft5406",//.name = TPD_DEVICE,
//	 .owner = THIS_MODULE,
  },
  .probe = tpd_probe,
...
static unsigned char CTPM_MUTTO_FW[]=
{
//#include "YarisXXL_5406_ID53_V16_20131203_app.i"
  #include "YarisXXL_5406_ID53_V1B_20140618_app.i"
//#include "YarisXXL_5406_ID53_V1E_20140902_app.i"

};

#ifdef YARISXXL_TV_VERSION_CHECK
static unsigned char CTPM_MUTTO_FW_TV_VERSION[]=
{
    #include "YarisXXL_5406_ID53_V1A_20140327_app.i"
};
#endif
...
static u8 fts_ctpm_get_upg_ver(void)
{
	...
	if (tp_factory_id_cmd == TS_TYPE_MUTTO) {
#ifdef YARISXXL_TV_VERSION_CHECK
	    if(tv_version == 1) // TV version
	    {
		    ui_sz = sizeof(CTPM_MUTTO_FW_TV_VERSION);
		    if (ui_sz > 2) {
		    printk("[motto]TV version fts_ctpm_get_upg_ver ui_sz,value1 value2 is %4x %2x %2x xxxxxxxxxx\n", \
			    ui_sz,CTPM_MUTTO_FW_TV_VERSION[ui_sz - 2],CTPM_MUTTO_FW_TV_VERSION[ui_sz - 1]);
			    return CTPM_MUTTO_FW_TV_VERSION[ui_sz - 2];
		    } else {
			    /*TBD, error handling?*/
...
#endif
        //NOT TV version
        ui_sz = sizeof(CTPM_MUTTO_FW);
        if (ui_sz > 2) {
            printk("[motto]fts_ctpm_get_upg_ver ui_sz,value1 value2 is %4x %2x %2x xxxxxxxxxx\n", \
            ui_sz,CTPM_MUTTO_FW[ui_sz - 2],CTPM_MUTTO_FW[ui_sz - 1]);
            return CTPM_MUTTO_FW[ui_sz - 2];
        } else {
            /*TBD, error handling?*/
            return 0xff; /*default value*/
        }
...
#ifdef YARISXXL_TV_VERSION_CHECK
        if(tv_version == 1){
		    pbt_buf = CTPM_MUTTO_FW_TV_VERSION;
		    /*call the upgrade function*/
		    i_ret =  fts_ctpm_fw_upgrade(pbt_buf,sizeof(CTPM_MUTTO_FW_TV_VERSION));
		}
		else{
#endif
            pbt_buf = CTPM_MUTTO_FW;
            /*call the upgrade function*/
		    i_ret =  fts_ctpm_fw_upgrade(pbt_buf,sizeof(CTPM_MUTTO_FW));
#ifdef YARISXXL_TV_VERSION_CHECK
        }
#endif
		if (i_ret != 0)	{
...
static void YarisXXL_TV_Check()
{
	...
	for(checktimes = 0; checktimes < 3; checktimes++){
		...
		if (ret) {
			printk("[FT5406] IMM_GetOneChannelValue failed! get mipi lane info failed! checktimes: %d\n", checktimes);
		}
		else
		{
			real_val = (rawvalue * 1800)/4096;
			if (real_val > 0 && real_val < 500) { // TV version
				tv_version = 1; //--
				printk("[FT5406] is TV version, checktimes: %d\n", checktimes);
				break;
			}
			printk("[FT5406] NOT TV version, checktimes: %d\n", checktimes);
		}
	}
}
#endif
...
#ifdef USB_CHARGE_DETECT
void tpd_usb_plugin(int plugin,int force)
{
	 int ret = -1;

     if( !force )
	     if(b_usb_plugin == plugin) return;

     if( ( plugin == 0 ) || ( plugin == 1 ))// 1: usb plugin , 0:usb plugout
	     b_usb_plugin = plugin;
	 else
	 	return ;
...
//eint close, eint status
	...
	mt_eint_set_sens(CUST_EINT_TOUCH_PANEL_NUM, 0);//FIXME ydyu for compile ok
...
//dummy delay
	mdelay(5);
//set up vdd
	hwPowerOn(TPD_POWER_SOURCE_CUSTOM, VOL_2800, "TP");//modify by ydyu, change TP VDD fron pre-proto
//delay 5ms
	mdelay(10);
//set up rst
	mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ONE);
...
retry_update:
	if (update_retry > 0) {
	hwPowerDown(MT65XX_POWER_LDO_VRF28_2,"TP");
	msleep(20);	
	hwPowerOn(MT65XX_POWER_LDO_VRF28_2, VOL_3000, "TP");
	msleep(20);
	}
...
if (update_retry == 0) {
	...
	if (tp_factory_id_cmd == 0xa8)  //for tp-upgrade failed plan.
		tp_factory_id_cmd = TS_TYPE_MUTTO;
	...
	if (((chip_fw_ver == 0xa6) || (chip_fw_ver < firmware_ver)) && (firmware_ver != 0xFF)) {
		...
		printk("xxxxxxxxxafter upgrade : new firmware is %2xxxxxxxxxxx\n", chip_fw_ver);
	}
#endif
...
//ret = ft5x0x_i2c_Read(client, NULL, 0, buf, length);
	...
	/*ret=i2c_master_recv(i2c_client, buf, length);*/
	ret = ft5x0x_i2c_Read(client, NULL, 0, buf, length);
	if (ret < 0)
		pr_err("msg %s i2c read error: %d\n", __func__, ret);
...
static int ft_rw_iic_drv_mywrite(struct i2c_client *client, u8 *buf, int length)
{
	...
	/*ret=i2c_master_send(i2c_client,buf, length);*/
	ret = ft5x0x_i2c_Write(i2c_client, buf, length);
	if(ret<0)
...
static int ft_rw_iic_drv_RDWR(struct i2c_client *client, unsigned long arg)
{
    ...
    printk("leibo i2c_rw_queue.queuenum = %d\n", i2c_rw_queue.queuenum);
	for (i=0; i< i2c_rw_queue.queuenum; i++) {
	    printk("leibo i2c_rw_msg[i].flag = %d, i2c_rw_msg[i].length = %d\n", i2c_rw_msg[i].flag, i2c_rw_msg[i].length);
		if (i2c_rw_msg[i].flag) {
   	   		ret = ft_rw_iic_drv_myread(client,
					i2c_rw_msg[i].buf, i2c_rw_msg[i].length);
				//ret = -1;//ret = byte_read(i2c_rw_msg[i].buf, i2c_rw_msg[i].length);	
			if (ret>=0)
   	   			ret = copy_to_user(data_ptrs[i], i2c_rw_msg[i].buf, i2c_rw_msg[i].length);
   	   	}
		else
			ft_rw_iic_drv_mywrite(client,
					i2c_rw_msg[i].buf, i2c_rw_msg[i].length);
					//ret = byte_write(i2c_rw_msg[i].buf, i2c_rw_msg[i].length);
	}
