...
{
    PK_DBG("[liwenyuan_fl][flashlight_release] E\n");
  // 		dump_stack();
    if (g_pFlashlightFunc) {
        g_pFlashlightFunc->flashlight_release(0);
 //       g_pFlashlightFunc = NULL;  //liwenyuan@tcl 20130704 modify the release flow for the flashlight light can't work at some time
    }
    return 0;
}
