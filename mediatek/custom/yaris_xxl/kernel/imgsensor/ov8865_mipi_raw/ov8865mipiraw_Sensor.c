...
static int OV8865_SHUTTER_MARGIN = 4;
static int OV8865_GAIN_BASE = 128;
static int OV8865_AUTOFLICKER_OFFSET_30 = 296;
static int OV8865_AUTOFLICKER_OFFSET_15 = 146;
static int OV8865_PREVIEW_PCLK = 74400000;
static int OV8865_VIDEO_PCLK = 148800000;
static int OV8865_CAPTURE_PCLK = 148800000;
static int OV8865_MAX_FPS_PREVIEW = 300;
static int OV8865_MAX_FPS_VIDEO = 300;
static int OV8865_MAX_FPS_CAPTURE = 300;
static int OV8865_IMAGE_SENSOR_PV_WIDTH = 1632;
static int OV8865_IMAGE_SENSOR_PV_HEIGHT = 1224;
static int OV8865_IMAGE_SENSOR_VIDEO_WIDTH = 3264;
static int OV8865_IMAGE_SENSOR_VIDEO_HEIGHT = 1836;
static int OV8865_IMAGE_SENSOR_FULL_WIDTH = 3264;
static int OV8865_IMAGE_SENSOR_FULL_HEIGHT = 2448;
static int OV8865_FULL_X_START = 2;
static int OV8865_FULL_Y_START = 0;
static int OV8865_PV_X_START = 2;
static int OV8865_PV_Y_START = 0;
static int OV8865_VIDEO_X_START = 2;
static int OV8865_VIDEO_Y_START = 0;
static int OV8865_MAX_ANALOG_GAIN = 8;
static int OV8865_MIN_ANALOG_GAIN = 1;
static int OV8865_PV_PERIOD_PIXEL_NUMS = 0x0783;
static int OV8865_PV_PERIOD_LINE_NUMS = 0x04E0;
static int OV8865_VIDEO_PERIOD_PIXEL_NUMS = 0x0A16;
static int OV8865_VIDEO_PERIOD_LINE_NUMS = 0x0742;
static int OV8865_FULL_PERIOD_PIXEL_NUMS = 0x07d8;
static int OV8865_FULL_PERIOD_LINE_NUMS = 0x09d8;

void set_2lane_param(void)
{
	OV8865_SHUTTER_MARGIN = 4;
	OV8865_GAIN_BASE = 128;
	OV8865_AUTOFLICKER_OFFSET_30 = 296;
	OV8865_AUTOFLICKER_OFFSET_15 = 146;
	OV8865_PREVIEW_PCLK = 74400000;
	OV8865_VIDEO_PCLK = 74400000;
	OV8865_CAPTURE_PCLK = 74400000;
	OV8865_MAX_FPS_PREVIEW = 300;
	OV8865_MAX_FPS_VIDEO = 300;
	OV8865_MAX_FPS_CAPTURE = 150;
	OV8865_IMAGE_SENSOR_PV_WIDTH = 1632;
	OV8865_IMAGE_SENSOR_PV_HEIGHT = 1224;
	OV8865_IMAGE_SENSOR_VIDEO_WIDTH = 1632;
	OV8865_IMAGE_SENSOR_VIDEO_HEIGHT = 1224;
	OV8865_IMAGE_SENSOR_FULL_WIDTH = 3264;
	OV8865_IMAGE_SENSOR_FULL_HEIGHT = 2448;
	OV8865_FULL_X_START = 2;
	OV8865_FULL_Y_START = 0;
	OV8865_PV_X_START = 2;
	OV8865_PV_Y_START = 0;
	OV8865_VIDEO_X_START = 2;
	OV8865_VIDEO_Y_START = 0;
	OV8865_MAX_ANALOG_GAIN = 8;
	OV8865_MIN_ANALOG_GAIN = 1;
	OV8865_PV_PERIOD_PIXEL_NUMS = 0x0783;
	OV8865_PV_PERIOD_LINE_NUMS = 0x04E0;
	OV8865_VIDEO_PERIOD_PIXEL_NUMS = 0x0783;
	OV8865_VIDEO_PERIOD_LINE_NUMS = 0x04E0;
	OV8865_FULL_PERIOD_PIXEL_NUMS = 0x07d8;
	OV8865_FULL_PERIOD_LINE_NUMS = 0x09d8;
}

//static int OV8865_FeatureControl_PERIOD_PixelNum = OV8865_PV_PERIOD_PIXEL_NUMS;
//static int OV8865_FeatureControl_PERIOD_LineNum = OV8865_PV_PERIOD_LINE_NUMS;
static int OV8865_FeatureControl_PERIOD_PixelNum = 0x0783;
static int OV8865_FeatureControl_PERIOD_LineNum = 0x04E0;
...
#define GOLDEN_RG_RATIO 0x124  //R/G 85.95  =86=0x56  0108 Jouny
#define GOLDEN_BG_RATIO 0x119  //B/G  88.67 =89=0x59  0108 Jouny
//*******golden sample value: module vendor provide********//
...
kal_bool OV8865OTPWbPregain()
{
	int i, address, temp, index;
	u8 wb[5];
	u32 R_gain, G_gain, B_gain, nR_G_gain, nB_G_gain, nG_G_gain, nBase_gain;
	u32 rg, bg, light_rg, light_bg;
	...
	if(i > 3) {
		OV8865DB("No valid WB OTP data!\n");
		return KAL_FALSE;
	}

	OV8865_write_cmos_sensor(0x3d84, 0xC0);
	OV8865_write_cmos_sensor(0x3d88, 0x70);
	OV8865_write_cmos_sensor(0x3d89, 0x21);
	OV8865_write_cmos_sensor(0x3d8a, 0x70);
	OV8865_write_cmos_sensor(0x3d8b, 0x2f);
	...
	for(i = 0; i < 5; i++)
		wb[i] = (u8)OV8865_read_cmos_sensor(address + i);
		
	//truly wgs 20131220
	rg =(wb[0] << 2) + ((wb[4] >>6) & 0x03);
	bg=(wb[1] << 2) + ((wb[4] >> 4) & 0x03);

	for (i=0x7021; i<=0x702f; i++)
		OV8865_write_cmos_sensor(i, 0x00);

	//calculate
	
	nR_G_gain = (GOLDEN_RG_RATIO*1000) / rg;
	nB_G_gain = (GOLDEN_BG_RATIO*1000) / bg;
	nG_G_gain = 1000;
	...
	printk("[ov8865]R_gain = %x, B_gain = %x, G_gain = %x\n", R_gain, B_gain, G_gain);
	if(R_gain > 0x400){
...
kal_bool OV8865OTPShading()
{
	...
	OV8865DB("lenc otp i=%d\n", i);
	if(i > 3) {
		OV8865DB("hujl@debug:No valid LSC OTP data!\n");
		return KAL_FALSE;
	}

	OV8865_write_cmos_sensor(0x3d84, 0xC0);
	OV8865_write_cmos_sensor(0x3d88, 0x70);
	OV8865_write_cmos_sensor(0x3d89, 0x3b);
	OV8865_write_cmos_sensor(0x3d8a, 0x70);
	OV8865_write_cmos_sensor(0x3d8b, 0xf4);
	...
	for (i = 0; i < 62; i++)
		lenc[i] = (u8)OV8865_read_cmos_sensor(address + i);

	for (i=0x703b; i<=0x70f4; i++)
		OV8865_write_cmos_sensor(i, 0x00);
	...
	for (i = 0; i < 62; i++)
		OV8865_write_cmos_sensor(0x5800 + i, lenc[i]);

	return KAL_TRUE;
}

static void OnReadOTPOV8865(void)
{

	 OV8865DB("OnReadOTPOV8865 ENTER :\n ");

//read LSC parameter
	if(OV8865OTPShading())
	{
		OV8865DB("ov8865 lenc otp ok!!\n");
	}else{
		OV8865DB("ov8865 lenc otp fail!!\n");
	}

//read AWB data
	if(OV8865OTPWbPregain())
	{
		OV8865DB("ov8865 awb otp ok!!\n");
	}else{
		OV8865DB("ov8865 awb otp fail!!\n");
	}
...
UINT32 OV8865GetSensorID(UINT32 *sensorID)
{
    ...
    if (*sensorID != OV8865_SENSOR_ID) {
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
//	printk("hujl@debug:sensorID = %x\n", *sensorID);
//	*sensorID = OV8865_SENSOR_ID;
    }
    return ERROR_NONE;
}
