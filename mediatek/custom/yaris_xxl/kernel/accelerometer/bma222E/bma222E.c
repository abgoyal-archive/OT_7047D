...
/*----------------------------------------------------------------------------*/
static const struct i2c_device_id bma222_i2c_id[] =
    { {BMA222_DEV_NAME, 0}, {} };
...
/*----------------------------------------------------------------------------*/
static int bma222_i2c_probe(struct i2c_client *client,
			    const struct i2c_device_id *id);
static int bma222_i2c_remove(struct i2c_client *client);
static int bma222_i2c_detect(struct i2c_client *client,
...
static void BMA222_power(struct acc_hw *hw, unsigned int on)
{
	...
	if (hw->power_id != POWER_NONE_MACRO)	// have externel LDO
	{
		...
		{
			if (!hwPowerOn
			    (hw->power_id, hw->power_vol, "BMA222")) {
...
/*----------------------------------------------------------------------------*/
static int BMA222_ReadData(struct i2c_client *client,
			   s16 data[BMA222_AXES_NUM])
{
	...
	if (NULL == client) {
		err = -EINVAL;
	} else if ((err = bma_i2c_read_block(client, addr, buf, 0x05)) !=
		   0) {
		GSE_ERR("error: %d\n", err);
	} else {
		...
		if (atomic_read(&priv->trace) & ADX_TRC_RAWDATA) {
			GSE_LOG
			    ("[%08X %08X %08X] => [%5d %5d %5d] before\n",
			     data[BMA222_AXIS_X], data[BMA222_AXIS_Y],
			     data[BMA222_AXIS_Z], data[BMA222_AXIS_X],
		...
		if (atomic_read(&priv->filter)) {
			if (atomic_read(&priv->fir_en)
				...
				if (priv->fir.num < firlen) {
					...
					priv->fir.sum[BMA222_AXIS_X] -=
					    priv->fir.
					    raw[idx][BMA222_AXIS_X];
					priv->fir.sum[BMA222_AXIS_Y] -=
					    priv->fir.
					    raw[idx][BMA222_AXIS_Y];
					priv->fir.sum[BMA222_AXIS_Z] -=
					    priv->fir.
					    raw[idx][BMA222_AXIS_Z];
...
static int BMA222_ReadOffset(struct i2c_client *client,
			     s8 ofs[BMA222_AXES_NUM])
{
	int err;
	err = 0;
...
#else
	if ((err =
	     bma_i2c_read_block(client, BMA222_REG_OFSX, ofs,
				BMA222_AXES_NUM))) {
		GSE_ERR("error: %d\n", err);
	}
#endif
...
/*----------------------------------------------------------------------------*/
static int BMA222_ReadCalibration(struct i2c_client *client,
				  int dat[BMA222_AXES_NUM])
{
	...
	mul =
	    obj->reso->sensitivity / bma222_offset_resolution.sensitivity;
#endif

	dat[obj->cvt.map[BMA222_AXIS_X]] =
	    obj->cvt.sign[BMA222_AXIS_X] * (obj->offset[BMA222_AXIS_X] *
					    mul +
					    obj->cali_sw[BMA222_AXIS_X]);
	dat[obj->cvt.map[BMA222_AXIS_Y]] =
	    obj->cvt.sign[BMA222_AXIS_Y] * (obj->offset[BMA222_AXIS_Y] *
					    mul +
					    obj->cali_sw[BMA222_AXIS_Y]);
	dat[obj->cvt.map[BMA222_AXIS_Z]] =
	    obj->cvt.sign[BMA222_AXIS_Z] * (obj->offset[BMA222_AXIS_Z] *
					    mul +
...
/*----------------------------------------------------------------------------*/
static int BMA222_ReadCalibrationEx(struct i2c_client *client,
				    int act[BMA222_AXES_NUM],
				    int raw[BMA222_AXES_NUM])
{
	...
	act[obj->cvt.map[BMA222_AXIS_X]] =
	    obj->cvt.sign[BMA222_AXIS_X] * raw[BMA222_AXIS_X];
	act[obj->cvt.map[BMA222_AXIS_Y]] =
	    obj->cvt.sign[BMA222_AXIS_Y] * raw[BMA222_AXIS_Y];
	act[obj->cvt.map[BMA222_AXIS_Z]] =
	    obj->cvt.sign[BMA222_AXIS_Z] * raw[BMA222_AXIS_Z];

	return 0;
}

/*----------------------------------------------------------------------------*/
static int BMA222_WriteCalibration(struct i2c_client *client,
				   int dat[BMA222_AXES_NUM])
{
	struct bma222_i2c_data *obj = i2c_get_clientdata(client);
	int err = 0;
	...
	GSE_LOG
	    ("OLDOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n",
	     ...
	     obj->offset[BMA222_AXIS_Z], obj->cali_sw[BMA222_AXIS_X],
	     obj->cali_sw[BMA222_AXIS_Y], obj->cali_sw[BMA222_AXIS_Z]);
	...
	GSE_LOG("UPDATE: (%+3d %+3d %+3d)\n",
		dat[BMA222_AXIS_X], dat[BMA222_AXIS_Y],
		dat[BMA222_AXIS_Z]);

#ifdef SW_CALIBRATION
	obj->cali_sw[BMA222_AXIS_X] =
	    obj->cvt.sign[BMA222_AXIS_X] *
	    (cali[obj->cvt.map[BMA222_AXIS_X]]);
	obj->cali_sw[BMA222_AXIS_Y] =
	    obj->cvt.sign[BMA222_AXIS_Y] *
	    (cali[obj->cvt.map[BMA222_AXIS_Y]]);
	obj->cali_sw[BMA222_AXIS_Z] =
	    obj->cvt.sign[BMA222_AXIS_Z] *
	    (cali[obj->cvt.map[BMA222_AXIS_Z]]);
#else
	int divisor = obj->reso->sensitivity / lsb;	//modified
	obj->offset[BMA222_AXIS_X] =
	    (s8) (obj->cvt.sign[BMA222_AXIS_X] *
		  (cali[obj->cvt.map[BMA222_AXIS_X]]) / (divisor));
	obj->offset[BMA222_AXIS_Y] =
	    (s8) (obj->cvt.sign[BMA222_AXIS_Y] *
		  (cali[obj->cvt.map[BMA222_AXIS_Y]]) / (divisor));
	obj->offset[BMA222_AXIS_Z] =
	    (s8) (obj->cvt.sign[BMA222_AXIS_Z] *
		  (cali[obj->cvt.map[BMA222_AXIS_Z]]) / (divisor));
	...
	GSE_LOG
	    ("NEWOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n",
	     obj->offset[BMA222_AXIS_X] * divisor +
	     obj->cali_sw[BMA222_AXIS_X],
	     obj->offset[BMA222_AXIS_Y] * divisor +
	     obj->cali_sw[BMA222_AXIS_Y],
	     obj->offset[BMA222_AXIS_Z] * divisor +
	     obj->cali_sw[BMA222_AXIS_Z], obj->offset[BMA222_AXIS_X],
	     obj->offset[BMA222_AXIS_Y], obj->offset[BMA222_AXIS_Z],
	     obj->cali_sw[BMA222_AXIS_X], obj->cali_sw[BMA222_AXIS_Y],
	     obj->cali_sw[BMA222_AXIS_Z]);

	if ((err =
	     hwmsen_write_block(obj->client, BMA222_REG_OFSX, obj->offset,
				BMA222_AXES_NUM))) {
...
static int BMA222_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	res =
	    bma_i2c_write_block(client, BMA222_REG_POWER_CTL, databuf,
				0x1);
	if (res < 0) {
...
static int BMA222_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	if (bma_i2c_read_block
	    (client, BMA222_REG_DATA_FORMAT, databuf, 0x01)) {
	...
	res =
	    bma_i2c_write_block(client, BMA222_REG_DATA_FORMAT, databuf,
				0x1);
	if (res < 0) {
		return BMA222_ERR_I2C;
	}
...
static int BMA222_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	res =
	    bma_i2c_write_block(client, BMA222_REG_BW_RATE, databuf, 0x1);
	if (res < 0) {
		return BMA222_ERR_I2C;
	}
	mdelay(1);
...
/*----------------------------------------------------------------------------*/
static int BMA222_ReadChipInfo(struct i2c_client *client, char *buf,
			       int bufsize)
{
	u8 databuf[10];
...
/*----------------------------------------------------------------------------*/
static int BMA222_ReadSensorData(struct i2c_client *client, char *buf,
				 int bufsize)
{
	struct bma222_i2c_data *obj =
	    (struct bma222_i2c_data *) i2c_get_clientdata(client);
	u8 databuf[20];
	...
	if (sensor_power == FALSE) {
		GSE_ERR
		    ("BMA222_ReadSensorData bad operation sensor_power = %d!\n",
		     sensor_power);
	...
	if ((res = BMA222_ReadData(client, obj->data)) != 0) {
		...
		acc[obj->cvt.map[BMA222_AXIS_X]] =
		    obj->cvt.sign[BMA222_AXIS_X] *
		    obj->data[BMA222_AXIS_X];
		acc[obj->cvt.map[BMA222_AXIS_Y]] =
		    obj->cvt.sign[BMA222_AXIS_Y] *
		    obj->data[BMA222_AXIS_Y];
		acc[obj->cvt.map[BMA222_AXIS_Z]] =
		    obj->cvt.sign[BMA222_AXIS_Z] *
		    obj->data[BMA222_AXIS_Z];
		...
		acc[BMA222_AXIS_X] =
		    acc[BMA222_AXIS_X] * GRAVITY_EARTH_1000 /
		    obj->reso->sensitivity;
		acc[BMA222_AXIS_Y] =
		    acc[BMA222_AXIS_Y] * GRAVITY_EARTH_1000 /
		    obj->reso->sensitivity;
		acc[BMA222_AXIS_Z] =
		    acc[BMA222_AXIS_Z] * GRAVITY_EARTH_1000 /
		    obj->reso->sensitivity;



		sprintf(buf, "%04x %04x %04x", acc[BMA222_AXIS_X],
			acc[BMA222_AXIS_Y], acc[BMA222_AXIS_Z]);
...
static int BMA222_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	if ((res = BMA222_ReadData(client, obj->data))) {
		...
		sprintf(buf, "BMA222_ReadRawData %04x %04x %04x",
			obj->data[BMA222_AXIS_X], obj->data[BMA222_AXIS_Y],
			obj->data[BMA222_AXIS_Z]);
...
#if 0
static ssize_t gsensor_init(struct device_driver *ddri, char *buf,
			    size_t count)
{
	struct i2c_client *client = bma222_i2c_client;
	char strbuf[BMA222_BUFSIZE];
...
#if 0
static ssize_t show_sensorrawdata_value(struct device_driver *ddri,
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
	...
	if ((err = BMA222_ReadOffset(client, obj->offset))) {
		...
		mul =
		    obj->reso->sensitivity /
		    bma222_offset_resolution.sensitivity;
		len +=
		    snprintf(buf + len, PAGE_SIZE - len,
			     "[HW ][%d] (%+3d, %+3d, %+3d) : (0x%02X, 0x%02X, 0x%02X)\n",
			     mul, obj->offset[BMA222_AXIS_X],
			     obj->offset[BMA222_AXIS_Y],
			     obj->offset[BMA222_AXIS_Z],
			     obj->offset[BMA222_AXIS_X],
			     obj->offset[BMA222_AXIS_Y],
		...
		len +=
		    snprintf(buf + len, PAGE_SIZE - len,
			     ...
			     obj->offset[BMA222_AXIS_X] * mul +
			     obj->cali_sw[BMA222_AXIS_X],
			     obj->offset[BMA222_AXIS_Y] * mul +
			     obj->cali_sw[BMA222_AXIS_Y],
			     obj->offset[BMA222_AXIS_Z] * mul +
			     obj->cali_sw[BMA222_AXIS_Z],
			     tmp[BMA222_AXIS_X], tmp[BMA222_AXIS_Y],
			     tmp[BMA222_AXIS_Z]);
	...
	if (atomic_read(&obj->firlen)) {
		...
		for (idx = 0; idx < len; idx++) {
			GSE_LOG("[%5d %5d %5d]\n",
				...
				obj->fir.raw[idx][BMA222_AXIS_Y],
				obj->fir.raw[idx][BMA222_AXIS_Z]);
		}

		GSE_LOG("sum = [%5d %5d %5d]\n",
			obj->fir.sum[BMA222_AXIS_X],
			obj->fir.sum[BMA222_AXIS_Y],
			obj->fir.sum[BMA222_AXIS_Z]);
		GSE_LOG("avg = [%5d %5d %5d]\n",
			obj->fir.sum[BMA222_AXIS_X] / len,
			obj->fir.sum[BMA222_AXIS_Y] / len,
			obj->fir.sum[BMA222_AXIS_Z] / len);
	}
...
static ssize_t show_status_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj->hw) {
		...
		len +=
		    snprintf(buf + len, PAGE_SIZE - len, "CUST: NULL\n");
	}
	return len;
}

/*----------------------------------------------------------------------------*/
static ssize_t show_power_status_value(struct device_driver *ddri,
				       char *buf)
{
...
static DRIVER_ATTR(sensordata, S_IWUSR | S_IRUGO, show_sensordata_value,
		   NULL);
static DRIVER_ATTR(cali, S_IWUSR | S_IRUGO, show_cali_value,
		   store_cali_value);
...
static int bma222_create_attr(struct device_driver *driver)
{
	int idx, err = 0;
	int num =
	    (int) (sizeof(bma222_attr_list) / sizeof(bma222_attr_list[0]));
	if (driver == NULL) {
		return -EINVAL;
	}
	...
	for (idx = 0; idx < num; idx++) {
		if ((err =
		     driver_create_file(driver, bma222_attr_list[idx]))) {
...
/*----------------------------------------------------------------------------*/
int gsensor_operate(void *self, uint32_t command, void *buff_in,
		    ...
		    int *actualout)
{
	...
	case SENSOR_ENABLE:
		if ((buff_in == NULL) || (size_in < sizeof(int))) {
			...
			mutex_lock(&bma222_op_mutex);
			GSE_LOG
			    ("Gsensor device enable function enable = %d, sensor_power = %d!\n",
			     value, sensor_power);
			if (((value == 0) && (sensor_power == false))
				...
				enable_status = !sensor_power;
				err =
				    BMA222_SetPowerMode(priv->client,
							!sensor_power);
				GSE_LOG
				    ("Gsensor not in suspend BMA222_SetPowerMode!, enable_status = %d\n",
				     enable_status);
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL)
			...
			BMA222_ReadSensorData(priv->client, buff,
					      BMA222_BUFSIZE);
				  ...
				  unsigned long arg)
{
	...
	case GSENSOR_IOCTL_READ_GAIN:
		...
		if (copy_to_user
		    (data, &gsensor_gain, sizeof(GSENSOR_VECTOR3D))) {
			err = -EFAULT;
			break;
		}
		break;
	...
	case GSENSOR_IOCTL_GET_CALI:
		...
		sensor_data.x =
		    cali[BMA222_AXIS_X] * GRAVITY_EARTH_1000 /
		    obj->reso->sensitivity;
		sensor_data.y =
		    cali[BMA222_AXIS_Y] * GRAVITY_EARTH_1000 /
		    obj->reso->sensitivity;
		sensor_data.z =
		    cali[BMA222_AXIS_Z] * GRAVITY_EARTH_1000 /
		    obj->reso->sensitivity;
...
{
	struct bma222_i2c_data *obj =
	    container_of(h, struct bma222_i2c_data, early_drv);
	int err;
	...
	if (bma_i2c_read_block
	    (obj->client, BMA222_REG_POWER_CTL, databuf, 0x01)) {
		GSE_ERR("read power ctl register err!\n");
		mutex_unlock(&bma222_op_mutex);
		return BMA222_ERR_I2C;
	}
	if (databuf[0] == 0xff)	//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
		GSE_LOG
		    ("before BMA222_SetPowerMode in suspend databuf = 0x%x\n",
		     databuf[0]);
	if ((err = BMA222_SetPowerMode(obj->client, false))) {
		GSE_ERR("write power control fail!!\n");
		mutex_unlock(&bma222_op_mutex);
	...
	if (databuf[0] == 0xff)	//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
		GSE_LOG
		    ("after BMA222_SetPowerMode suspend err databuf = 0x%x\n",
		     databuf[0]);
	sensor_suspend = 1;
	mutex_unlock(&bma222_op_mutex);
	BMA222_power(obj->hw, 0);
			    ...
			    const struct i2c_device_id *id)
{
	...
	for (retry = 0; retry < 3; retry++) {
		if ((err = bma222_init_client(new_client, 1))) {
			GSE_ERR
			    ("bma222_device init cilent fail time: %d\n",
			     retry);
			continue;
		}
	}
