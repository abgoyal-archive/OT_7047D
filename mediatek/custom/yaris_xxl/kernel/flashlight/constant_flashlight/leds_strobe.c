...
//@zhfan 2013-0825 register setting: see AS3647 datasheet for details
/*Current Set Register*/
#define CURRENT_SET_REG_ADDR        0x1
#define CURRENT_PERDUTY             5                         //delta current register value for one duty
#define LED_MAX_CURRENT             159                       //1A
#define ASSIST_LIGHT_MODE_MAX_DUTY  24                        //max duty for assist light mode, 25*5<0x7F<26*5
#define FLASH_MODE_MAX_DUTY         31                        //max duty for flash mode
#define GET_LED_CURRENT(duty)       ((duty+1)*CURRENT_PERDUTY)  //get led current
//for torch current level
#define LOW_CURRENT                 0x10
#define MID_CURRENT                 0x20
#define HIGH_CURRENT                0x2c
/*Flash Timer Register*/
#define FLASH_TIMER_REG_ADDR        0x5
#define FLASH_TIMEOUT               0xDC                      //1s
/*Control Register*/
#define CONTROL_REG_ADDR            0x6
//mode_setting
#define ASSIT_LIGHT_MODE            0x2
#define FLASH_MODE                  0x3
//out_on
#define OUT_ON                      0x8
#define OUT_OFF                     0x0
/*Strobe Signalling Register*/
#define STROBE_SIGNALLING_REG_ADDR  0X7
//strobe_type
#define LEVEL                       0x40
#define EDGE                        0x0
//strobe_on
#define STROBE_ON                   0x80
#define STROBE_OFF                  0x0
//@zhfan 2013-0825
...
int FL_Enable(void)
{
	char buf[2];
	int reg6;
	int mode_setting=ASSIT_LIGHT_MODE;
	PK_DBG(" FL_Enable\n");
	if(g_duty>ASSIST_LIGHT_MODE_MAX_DUTY)
		mode_setting=FLASH_MODE;
	reg6 = OUT_ON+mode_setting;
	buf[0]=CONTROL_REG_ADDR;
	buf[1]=reg6;
...
int FL_dim_duty(kal_uint32 duty)
{
    ...
    if(duty<0)
    	duty=0;
    if(duty>=FLASH_MODE_MAX_DUTY)
    	duty=FLASH_MODE_MAX_DUTY;
    g_duty=duty;
    //[begin]467220 zhfan 2013-6-10 modify the value for each duty
    reg1=GET_LED_CURRENT(duty);
    if(reg1>LED_MAX_CURRENT)
    	reg1=LED_MAX_CURRENT;
    //[end ]467220 zhfan  2013-6-10
    buf[0]=CURRENT_SET_REG_ADDR;
	buf[1]=reg1;
	//iWriteRegI2C(buf , 2, STROBE_DEVICE_ID);
	s4AS3647_write_byte(buf[0], buf[1]);
    return 0;
...
int FL_Init(void)
{
    ...
    buf[0]=FLASH_TIMER_REG_ADDR;
	buf[1]=FLASH_TIMEOUT;
	//iWriteRegI2C(buf , 2, STROBE_DEVICE_ID); //timeout 1s
	s4AS3647_write_byte(buf[0], buf[1]);

	buf[0]=STROBE_SIGNALLING_REG_ADDR;
	buf[1]=EDGE|STROBE_OFF;
...
{
    s4AS3647_write_byte(CURRENT_SET_REG_ADDR, val);
    s4AS3647_write_byte(CONTROL_REG_ADDR, ASSIT_LIGHT_MODE|OUT_ON);
    msleep(10);
}
...
{
    s4AS3647_write_byte(CURRENT_SET_REG_ADDR, val);
    s4AS3647_write_byte(CONTROL_REG_ADDR, FLASH_MODE|OUT_ON);
    msleep(10);
}
//[End]
...
ssize_t TORCH_select(void)
{
	...
	*/
	switch(strobe_width)
			{
			case 30:
				s4AS3647_write_byte(CURRENT_SET_REG_ADDR, LOW_CURRENT);
				break;
			case 60:
				s4AS3647_write_byte(CURRENT_SET_REG_ADDR, MID_CURRENT);
				break;
			case 99:
				s4AS3647_write_byte(CURRENT_SET_REG_ADDR, HIGH_CURRENT);
				break;
			default :
					break;
			}
...
ssize_t TORCH_Disable(void) {
	...
	*/
	s4AS3647_write_byte(CONTROL_REG_ADDR, ASSIT_LIGHT_MODE|OUT_OFF);
	    ...
	    case FLASHLIGHT_TORCH_SELECT:
			...
			if (arg && strobe_width){
				...
				g_strobe_On = TRUE;
			       ...
			       g_strobe_On = FALSE;    //liwenyuan@tcl 20130704  bug 480713 for g_strobe_On more precise 
				if(TORCH_Disable()) {
