...
#include "stk3x1x.h"
#define DRIVER_VERSION          "3.1.2.2 20131210 v2"
...
extern void mt65xx_eint_set_polarity(kal_uint8 eintno,
				     kal_bool ACT_Polarity);
extern void mt65xx_eint_set_hw_debounce(kal_uint8 eintno, kal_uint32 ms);
extern kal_uint32 mt65xx_eint_set_sens(kal_uint8 eintno, kal_bool sens);
extern void mt65xx_eint_registration(kal_uint8 eintno,
				     kal_bool Dbounce_En,
				     kal_bool ACT_Polarity,
...
extern void mt_eint_set_hw_debounce(unsigned int eint_num,
				    unsigned int ms);
...
extern void MT6516_EINT_Set_Polarity(kal_uint8 eintno,
				     kal_bool ACT_Polarity);
extern void MT6516_EINT_Set_HW_Debounce(kal_uint8 eintno, kal_uint32 ms);
extern kal_uint32 MT6516_EINT_Set_Sensitivity(kal_uint8 eintno,
					      kal_bool sens);
...
#ifdef STK_TUNE0
#define STK_MAX_MIN_DIFF	300	//380
#define STK_LT_N_CT	100	//250
#define STK_HT_N_CT	150	//330
...
static struct i2c_board_info __initdata i2c_stk3x1x =
    { I2C_BOARD_INFO("stk3x1x", (0x90 >> 1)) };

#else				/*  */
/*the adapter id & i2c address will be available in customization*/
static unsigned short stk3x1x_force[] =
    { 0x00, 0x00, I2C_CLIENT_END, I2C_CLIENT_END };
static const unsigned short *const stk3x1x_forces[] =
    { stk3x1x_force, NULL };
static struct i2c_client_address_data stk3x1x_addr_data = {
	.forces = stk3x1x_forces,
};
...
#endif				/*  */
/*----------------------------------------------------------------------------*/
static int stk3x1x_i2c_suspend(struct i2c_client *client,
			       pm_message_t msg);
...
/*----------------------------------------------------------------------------*/
int stk3x1x_get_addr(struct stk_alsps_hw *hw,
		     struct stk3x1x_i2c_addr *addr)
{
	if (!hw || !addr) {
		return -EFAULT;
			      ...
			      u8 * data, u8 len)
{
	...
	struct i2c_msg msgs[2] = {
		...
		{.addr = client->addr,.flags = I2C_M_RD,.len = len,.buf =
		 data,}
	};
	int err;
	if (!client)
...
/*----------------------------------------------------------------------------*/
int stk3x1x_master_recv(struct i2c_client *client, u16 addr, u8 * buf,
			int count)
{
...
/*----------------------------------------------------------------------------*/
int stk3x1x_master_send(struct i2c_client *client, u16 addr, u8 * buf,
			int count)
{
	int ret = 0, retry = 0;
	struct stk3x1x_priv *obj = i2c_get_clientdata(client);
...
int stk3x1x_write_sw_reset(struct i2c_client *client)
{
	...
	buf = 0x7F;
	ret =
	    stk3x1x_master_send(client, obj->addr.wait, (char *) &buf,
				sizeof(buf));
	if (ret < 0) {
	...
	buf = 0;
	ret =
	    stk3x1x_master_send(client, obj->addr.soft_reset,
				(char *) &buf, sizeof(buf));
	if (ret < 0) {
...
int stk3x1x_write_ps_high_thd(struct i2c_client *client, u16 thd)
{
	...
	ret =
	    stk3x1x_master_send(client, obj->addr.thdh2_ps, &(buf[1]), 1);
	if (ret < 0) {
		APS_ERR("WARNING: %d\n", ret);
		return -EFAULT;
	}
...
int stk3x1x_write_ps_low_thd(struct i2c_client *client, u16 thd)
{
	...
	ret =
	    stk3x1x_master_send(client, obj->addr.thdl2_ps, &(buf[1]), 1);
	if (ret < 0) {
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
	}
...
int stk3x1x_write_als_high_thd(struct i2c_client *client, u16 thd)
{
	...
	ret =
	    stk3x1x_master_send(client, obj->addr.thdh2_als, &(buf[1]), 1);
	if (ret < 0) {
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
	}
...
int stk3x1x_write_als_low_thd(struct i2c_client *client, u16 thd)
{
	...
	ret =
	    stk3x1x_master_send(client, obj->addr.thdl2_als, &(buf[1]), 1);
	if (ret < 0) {
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
	}
...
int stk3x1x_write_foffset(struct i2c_client *client, u16 ofset)
{
	...
	ret =
	    stk3x1x_master_send(client, obj->addr.data1_offset, &buf[0],
				1);
	if (ret < 0) {
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
	...
	ret =
	    stk3x1x_master_send(client, obj->addr.data2_offset, &(buf[1]),
				1);
	if (ret < 0) {
		APS_ERR("WARNING: %s: %d\n", __func__, ret);
		return -EFAULT;
...
int stk3x1x_write_aoffset(struct i2c_client *client, u16 ofset)
{
	...
	if (re_en) {
		...
		ret =
		    stk3x1x_master_send(client, obj->addr.state, &s_buf,
					1);
		if (ret < 0) {
			APS_ERR("write state = %d\n", ret);
			return -EFAULT;
...
static void stk3x1x_power(struct stk_alsps_hw *hw, unsigned int on)
{
	...
	if (hw->power_id != POWER_NONE_MACRO) {
		if (power_on == on) {
			...
			if (!hwPowerOn
			    (hw->power_id, hw->power_vol, "stk3x1x")) {
	...
	if (enable) {
		if (obj->hw->polling_mode_als) {
			...
			atomic_set(&obj->als_deb_end,
				   jiffies +
				   atomic_read(&obj->als_debounce) * HZ /
				   1000);
		}
	...
	if (enable) {
		...
		if (obj->hw->polling_mode_ps) {
			...
			atomic_set(&obj->ps_deb_end,
				   jiffies +
				   atomic_read(&obj->ps_debounce) * HZ /
				   1000);
		}
			...
			if (!validate_reg) {
				...
				msleep(4);
				if ((err =
				     stk3x1x_read_ps(obj->client,
						     &obj->ps))) {
					APS_ERR
					    ("stk3x1x read ps data: %d\n",
					     err);
					return err;
				}
				err =
				    stk3x1x_get_ps_value_only(obj,
							      obj->ps);
				if (err < 0) {
					APS_ERR
					    ("stk3x1x get ps value: %d\n",
					     err);
					return err;
				}

				else if (stk3x1x_obj->hw->
					 polling_mode_ps == 0) {
...
static int stk3x1x_clear_intr(struct i2c_client *client, u8 status,
			      u8 disable_flag)
{
	int err = 0;
	status =
	    status | (STK_FLG_ALSINT_MASK | STK_FLG_PSINT_MASK |
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
	...
	if ((err =
	     stk3x1x_write_state(obj->client,
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	int ret, diff;
	ps_enabled =
	    (atomic_read(&obj->state_val) & STK_STATE_EN_PS_MASK) ? true :
	    false;
...
static void stk_ps_tune0_work_func(struct work_struct *work)
{
	struct stk3x1x_priv *obj =
	    container_of(work, struct stk3x1x_priv, stk_ps_tune0_work);
...
static enum hrtimer_restart stk_ps_tune0_timer_func(struct hrtimer
						    *timer)
{
	struct stk3x1x_priv *obj =
	    container_of(timer, struct stk3x1x_priv, ps_tune0_timer);
...
static void stk3x1x_eint_work(struct work_struct *work)
{
	...
	if (((1 << STK_BIT_ALS) & obj->pending_intr)
		...
		if ((err =
		     hwmsen_get_interrupt_data(ID_LIGHT, &sensor_data))) {
			APS_ERR
			    ("call hwmsen_get_interrupt_data fail = %d\n",
			     err);
		}
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
...
#ifdef MT6516
	MT6516_EINT_Set_Sensitivity(CUST_EINT_ALS_NUM,
				    CUST_EINT_ALS_SENSITIVE);
	MT6516_EINT_Set_Polarity(CUST_EINT_ALS_NUM,
				 CUST_EINT_ALS_POLARITY);
	MT6516_EINT_Set_HW_Debounce(CUST_EINT_ALS_NUM,
				    CUST_EINT_ALS_DEBOUNCE_CN);
	MT6516_EINT_Registration(CUST_EINT_ALS_NUM,
	...
	mt65xx_eint_set_polarity(CUST_EINT_ALS_NUM,
				 CUST_EINT_ALS_POLARITY);
	mt65xx_eint_set_hw_debounce(CUST_EINT_ALS_NUM,
				    CUST_EINT_ALS_DEBOUNCE_CN);
	mt65xx_eint_registration(CUST_EINT_ALS_NUM,
				 CUST_EINT_ALS_DEBOUNCE_EN,
				 CUST_EINT_ALS_POLARITY,
				 stk3x1x_eint_func, 0);
	mt65xx_eint_unmask(CUST_EINT_ALS_NUM);
...
#if (defined(MT6582))
	mt_eint_set_hw_debounce(CUST_EINT_ALS_NUM,
				CUST_EINT_ALS_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINTF_TRIGGER_LOW,
			     stk3x1x_eint_func, 0);
	mt_eint_unmask(CUST_EINT_ALS_NUM);
...
static int stk3x1x_init_client(struct i2c_client *client)
{
	...
	if ((err =
	     stk3x1x_write_state(client, atomic_read(&obj->state_val)))) {
		APS_ERR("write stete error: %d\n", err);
		return err;
	}
	...
	if ((err =
	     stk3x1x_clear_intr(client, int_status,
				STK_FLG_PSINT_MASK |
				STK_FLG_ALSINT_MASK))) {
	...
	if ((err =
	     stk3x1x_write_als(client, atomic_read(&obj->alsctrl_val)))) {
...
static ssize_t stk3x1x_show_config(struct device_driver *ddri, char *buf)
{
	...
	res =
	    scnprintf(buf, PAGE_SIZE, "(%d %d %d %d %d %d)\n",
				    ...
				    const char *buf, size_t count)
{
	...
	if (6 ==
	    sscanf(buf, "%d %d %d %d %d %d", &retry, &als_deb, &mask,
		   &hthres, &lthres, &ps_deb)) {
...
static ssize_t stk3x1x_show_reg(struct device_driver *ddri, char *buf)
{
	...
	stk3x1x_write_ps_high_thd(stk3x1x_obj->client,
				  atomic_read
				  (&stk3x1x_obj->ps_high_thd_val));
	stk3x1x_write_ps_low_thd(stk3x1x_obj->client,
				 atomic_read
				 (&stk3x1x_obj->ps_low_thd_val));
	return 0;
}
				  ...
				  const char *buf, size_t count)
{
	...
	APS_LOG("send(%02X, %02X) = %d\n", addr, cmd,
		stk3x1x_master_send(stk3x1x_obj->client, (u16) addr,
				    &dat, sizeof(dat)));
	return count;
}
...
static ssize_t stk3x1x_show_allreg(struct device_driver *ddri, char *buf)
{
	...
	ret =
	    stk3x1x_master_recv(stk3x1x_obj->client, STK_PDT_ID_REG,
				&rbuf[25], 2);
	if (ret < 0) {
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
	...
	return scnprintf(buf, PAGE_SIZE,
			 ...
			 rbuf[10], rbuf[11], rbuf[12], rbuf[13], rbuf[14],
			 rbuf[15], rbuf[16], rbuf[17], rbuf[18],
			 rbuf[19], rbuf[20], rbuf[21], rbuf[22], rbuf[23],
...
static ssize_t stk3x1x_show_status(struct device_driver *ddri, char *buf)
{
	...
	if (stk3x1x_obj->hw) {
		...
		len +=
		    scnprintf(buf + len, PAGE_SIZE - len, "CUST: NULL\n");
	}
	len +=
	    scnprintf(buf + len, PAGE_SIZE - len,
	...
	len +=
	    scnprintf(buf + len, PAGE_SIZE - len,
		      ...
		      CUST_EINT_ALS_NUM, CUST_EINT_ALS_POLARITY,
		      CUST_EINT_ALS_DEBOUNCE_EN,
		      CUST_EINT_ALS_DEBOUNCE_CN);
	len +=
	    scnprintf(buf + len, PAGE_SIZE - len,
	...
	len +=
	    scnprintf(buf + len, PAGE_SIZE - len, "VER.: %s\n",
		      DRIVER_VERSION);
	memset(rbuf, 0, sizeof(rbuf));
	ret = stk3x1x_master_recv(stk3x1x_obj->client, 0, &rbuf[0], 7);
	if (ret < 0) {
...
/*----------------------------------------------------------------------------*/
static int read_int_from_buf(struct stk3x1x_priv *obj, const char *buf,
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_alslv(struct device_driver *ddri,
				   const char *buf, size_t count)
{
	if (!stk3x1x_obj) {
		APS_ERR("stk3x1x_obj is null!!\n");
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_alsval(struct device_driver *ddri,
				    const char *buf, size_t count)
{
	if (!stk3x1x_obj) {
		APS_ERR("stk3x1x_obj is null!!\n");
...
static ssize_t stk3x1x_show_psi_set(struct device_driver *ddri, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "psa = 0x%x, psi = 0x%x, psi_set = 0x%x\n",stk3x1x_obj->psa,stk3x1x_obj->psi,stk3x1x_obj->psi_set);
}
#endif				/*  */
/*----------------------------------------------------------------------------*/
...
#ifdef STK_TUNE0
static DRIVER_ATTR(psi_set, S_IWUSR | S_IRUGO, stk3x1x_show_psi_set, NULL);
static DRIVER_ATTR(cali, S_IWUSR | S_IRUGO, stk3x1x_show_cali, NULL);

#endif				/*  */
/*----------------------------------------------------------------------------*/
static struct driver_attribute *stk3x1x_attr_list[] =
    { &driver_attr_als, &driver_attr_ps, &driver_attr_ir,
	&driver_attr_trace,	/*trace log */
	&driver_attr_config, &driver_attr_alslv, &driver_attr_alsval,
	&driver_attr_status, &driver_attr_send, &driver_attr_recv,
	&driver_attr_allreg,
	//    &driver_attr_i2c,
	&driver_attr_reg,
#ifdef STK_TUNE0
...
static int stk3x1x_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		if ((err =
		     driver_create_file(driver, stk3x1x_attr_list[idx]))) {
...
static int stk3x1x_get_ps_value(struct stk3x1x_priv *obj, u16 ps)
{
	...
	val = (flag & STK_FLG_NF_MASK) ? 1 : 0;
	if ((err =
	     stk3x1x_clear_intr(obj->client, flag, STK_FLG_OUI_MASK))) {
		APS_ERR("fail: %d\n", err);
		return err;
	}
	if (atomic_read(&obj->ps_suspend)) {
...
static int32_t stk3x1x_get_ir_value(struct stk3x1x_priv *obj)
{
	...
	return word_data;
      irs_err_i2c_rw:if (re_enable_ps)
		stk3x1x_enable_ps(obj->client, 1, 0);
	return ret;
}
...
static long stk3x1x_unlocked_ioctl(struct file *file, unsigned int cmd,
				   unsigned long arg)
#else				/*  */
static int stk3x1x_ioctl(struct inode *inode, struct file *file,
...
static struct file_operations stk3x1x_fops = {
	...
	.owner = THIS_MODULE,
#endif				/*  */
	.open = stk3x1x_open,.release = stk3x1x_release,
#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,36))
...
/*----------------------------------------------------------------------------*/
static struct miscdevice stk3x1x_device = {.minor =
	    MISC_DYNAMIC_MINOR,.name = "als_ps",.fops = &stk3x1x_fops,
};
...
{				/*early_suspend is only applied for ALS */
	int err;
	struct stk3x1x_priv *obj =
	    container_of(h, struct stk3x1x_priv, early_drv);
...
int stk3x1x_ps_operate(void *self, uint32_t command, void *buff_in,
		       ...
		       int *actualout)
{
	...
	default:
		APS_ERR
		    ("proximity sensor operate function no this parameter %d!\n",
		     command);
		err = -1;
		break;
	}
...
int stk3x1x_als_operate(void *self, uint32_t command, void *buff_in,
			int size_in, void *buff_out, int size_out,
			int *actualout)
{
	int err = 0;
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL)
			...
			sensor_data = (hwm_sensor_data *) buff_out;
			if ((err =
			     stk3x1x_read_als(obj->client, &obj->als))) {
				err = -1;
			}
	...
	default:
		APS_ERR
		    ("light sensor operate function no this parameter %d!\n",
		     command);
		err = -1;
		break;
	}
			     ...
			     const struct i2c_device_id *id)
{
	...
	hrtimer_init(&obj->ps_tune0_timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
...
#if defined(CONFIG_HAS_EARLYSUSPEND)
	obj->early_drv.level =
	    EARLY_SUSPEND_LEVEL_DISABLE_FB - 1, obj->early_drv.suspend =
	    stk3x1x_early_suspend, obj->early_drv.resume =
	    stk3x1x_late_resume, register_early_suspend(&obj->early_drv);

#endif				/*  */
	APS_LOG("%s: OK\n", __FUNCTION__);
	return 0;
      exit_create_attr_failed:misc_deregister(&stk3x1x_device);
      exit_misc_device_register_failed: exit_init_failed:
	//i2c_detach_client(client);
	//      exit_kfree:
	kfree(obj);
      exit:stk3x1x_i2c_client = NULL;
...
/*----------------------------------------------------------------------------*/
static struct platform_driver stk3x1x_alsps_driver = {.probe =
	    stk3x1x_probe,.remove = stk3x1x_remove,.driver = {.name =
							      "als_ps",
